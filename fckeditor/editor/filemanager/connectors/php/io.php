...
{
	return isset($_GET['CurrentFolder']) ? $_GET['CurrentFolder']:'/' ;
}
...
EOF;
	...
	$rpl = array( '\\' => '\\\\', '"' => '\\"' ) ;
	echo 'window.parent.OnUploadCompleted(' . $errorNumber . ',"'.$GLOBALS['Config']['WebBasePath'].str_replace('../','',$fileUrl).'","' . strtr( $fileName, $rpl ) . '", "' . strtr( $customMsg, $rpl ) . '") ;' ;
	echo '</script>' ;
	exit ;
}
